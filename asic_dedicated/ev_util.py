...
class asic_tiling_generator():
    def __init__(self, input_dnn, hw_spec):
        ...
        for i in range(len(input_dnn)):
            ...
            self.ws_rf_gb_tiling_choices[-1]['ch_in_rf_gb_choices']=factor_n(input_dnn[i][1]['ch_in'][0]//ch_in_noc,3)
            ...
            self.ws_rf_gb_tiling_choices[-1]['ch_out_rf_gb_choices']=factor_n(input_dnn[i][1]['ch_out'][0]//ch_out_noc,3)
            ...
            self.ws_rf_gb_tiling_choices[-1]['col_out_rf_gb_choices']=factor_n(input_dnn[i][1]['col_out'][0],3)
            ...
            self.ws_rf_gb_tiling_choices[-1]['row_out_rf_gb_choices']=factor_n(input_dnn[i][1]['row_out'][0],3)
            ...
            self.ws_rf_gb_tiling_choices[-1]['batch_rf_gb_choices']=factor_n(input_dnn[i][1]['batch'][0],3)
        ...
        for i in range(len(input_dnn)):
            ...
            for pe_design in pe_designs[0:10]:
                ...
                self.os_rf_gb_tiling_choices[-1][-1]['ch_in_rf_gb_choices']=factor_n(input_dnn[i][1]['ch_in'][0],3)
                self.os_rf_gb_tiling_choices_num[-1][-1].append(len(self.os_rf_gb_tiling_choices[-1][-1]['ch_in_rf_gb_choices']))
                
                self.os_rf_gb_tiling_choices[-1][-1]['ch_out_rf_gb_choices']=factor_n(input_dnn[i][1]['ch_out'][0]//self.os_noc[-1][-1]['ch_out_noc'],3)
                self.os_rf_gb_tiling_choices_num[-1][-1].append(len(self.os_rf_gb_tiling_choices[-1][-1]['ch_out_rf_gb_choices']))
                
                self.os_rf_gb_tiling_choices[-1][-1]['col_out_rf_gb_choices']=factor_n(input_dnn[i][1]['col_out'][0]//self.os_noc[-1][-1]['col_out_noc'],3)
                self.os_rf_gb_tiling_choices_num[-1][-1].append(len(self.os_rf_gb_tiling_choices[-1][-1]['col_out_rf_gb_choices']))
                
                self.os_rf_gb_tiling_choices[-1][-1]['row_out_rf_gb_choices']=factor_n(input_dnn[i][1]['row_out'][0]//self.os_noc[-1][-1]['row_out_noc'],3)
                self.os_rf_gb_tiling_choices_num[-1][-1].append(len(self.os_rf_gb_tiling_choices[-1][-1]['row_out_rf_gb_choices']))
            
                self.os_rf_gb_tiling_choices[-1][-1]['col_kernel_rf_gb_choices']=factor_n(input_dnn[i][1]['col_kernel'][0],3)
                self.os_rf_gb_tiling_choices_num[-1][-1].append(len(self.os_rf_gb_tiling_choices[-1][-1]['col_kernel_rf_gb_choices']))    

                self.os_rf_gb_tiling_choices[-1][-1]['row_kernel_rf_gb_choices']=factor_n(input_dnn[i][1]['row_kernel'][0],3)
    ...
    def tiling_translation(self,layer,pe_array,pe_array_dim_choices,rf_gb_choices,rf_gb_choices_order):
        ...
        if pe_array==0:
            ...
            tiling_str['row_out_rf']=row_out_tiling[0]
            tiling_str['batch_rf']=batch_tiling[0]
            tiling_str['ch_in_gb']=ch_in_tiling[1]
            tiling_str['ch_out_gb']=ch_out_tiling[1]
            tiling_str['col_out_gb']=col_out_tiling[1]
            tiling_str['row_out_gb']=row_out_tiling[1]
            tiling_str['batch_gb']=batch_tiling[1]
            tiling_str['ch_in_dram']=ch_in_tiling[2]
            tiling_str['ch_out_dram']=ch_out_tiling[2]
            tiling_str['col_out_dram']=col_out_tiling[2]
            tiling_str['row_out_dram']=row_out_tiling[2]
            ...
            tiling_str['col_out_rf']=col_out_tiling[0]
            tiling_str['row_out_rf']=row_out_tiling[0]
            tiling_str['col_kernel_rf']=col_kernel_tiling[0]
            tiling_str['row_kernel_rf']=row_kernel_tiling[0]
            tiling_str['batch_rf']=batch_tiling[0]
            tiling_str['ch_in_gb']=ch_in_tiling[1]
            tiling_str['ch_out_gb']=ch_out_tiling[1]
            tiling_str['col_kernel_gb']=col_kernel_tiling[1]
            tiling_str['row_kernel_gb']=row_kernel_tiling[1]
            tiling_str['col_out_gb']=col_out_tiling[1]
            tiling_str['row_out_gb']=row_out_tiling[1]
            tiling_str['batch_gb']=batch_tiling[1]
            tiling_str['ch_in_dram']=ch_in_tiling[2]
            ...
            tiling_str['col_out_dram']=col_out_tiling[2]
            tiling_str['row_out_dram']=row_out_tiling[2]
            tiling_str['col_kernel_dram']=col_kernel_tiling[2]
...
# def fpga_tiling_generator(input_dnn,buffer_limit,dsp_limit,return_partitioned_space=False,bit_width=16): 
    ...
    # hw_spec=copy.deepcopy(hw_spec)
    # [input_size,output_size, weight_size]=data_type_sizes(input_dnn)
    # total_rf=hw_spec['rf_vol']
    # total_gb=hw_spec['gb_vol']
    # dsp_limit=hw_spec['num_pe']
    # #partition gb and rf proportionally towards each data type's size 
    # #in the future if we would like to do dynamic allocation we can use the following as arguments instead
    # input_rf=input_size*total_rf//(input_size+output_size+weight_size)
    # output_rf=output_size*total_rf//(input_size+output_size+weight_size)
    # weight_rf=weight_size*total_rf//(input_size+output_size+weight_size)
    # input_gb=input_size*total_gb//(input_size+output_size+weight_size)
    # output_gb=output_size*total_gb//(input_size+output_size+weight_size)
    # weight_gb=weight_size*total_gb//(input_size+output_size+weight_size)
    ...
    # row_kernel=[]
    # #collect all dimension information
    # ch_in.append(input_dnn[0][1]['ch_in'][0])
    # ch_out.append(input_dnn[0][1]['ch_out'][0])
    # row_out.append(input_dnn[0][1]['row_out'][0])
    # col_out.append(input_dnn[0][1]['col_out'][0])
    # row_kernel.append(input_dnn[0][1]['row_kernel'][0])
    # col_kernel.append(input_dnn[0][1]['col_kernel'][0])
    # col_kernel_noc=input_dnn[0][1]['col_kernel'][0]
    # row_kernel_noc=input_dnn[0][1]['row_kernel'][0]
    # ch_in_choices=sorted(r_factors(input_dnn[0][1]['ch_in'][0]))
    # ch_out_choices=sorted(r_factors(input_dnn[0][1]['ch_out'][0]))
    # incr_out=True
    # incr_out_index=0
    # incr_in_index=0
    # tmp_ch_in_noc=1
    # tmp_ch_out_noc=1
    # while col_kernel_noc*row_kernel_noc*tmp_ch_in_noc*tmp_ch_out_noc <= dsp_limit:
        # ch_in_noc=tmp_ch_in_noc
        # ch_out_noc=tmp_ch_out_noc
        # if incr_out:
            # tmp_ch_out_noc=ch_out_choices[incr_out_index]
