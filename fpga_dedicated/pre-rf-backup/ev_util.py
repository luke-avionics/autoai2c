...
#default_hw hardware specs fed to the estimator
default_hw={ \
    'gb_vol':108*1024*8, \
    'rf_vol':6832, \
    'num_pe':256, \
...
def arch_life(child,input_stride_list,hw_spec,df_order=None):
    ...
    for i in range(len(child)):
        ...
        #if design hw constraint exceeded break return extremely large penalty
        if layer_score > -9e12:
...
def pop_ranking(pop_list,score_board):
    ...
    tmp_pop=[]
    #maybe can get rid of the deepcopy; but for safety better not...
...
def lo_random_pop(layer_list=[10,10,7]):
    ...
    #  !!!! will be different in post_rf, will rf level is also excluded
    #layer_list: denoting the number of components in each memory level, with NoC level excluded (we have a lot of constraint on NoC level)
    #            RF level
    #            SRAM(GB) level
    #            DRAM level 
...
# NoC level template to be considered: enlarged since DAC
...
#ignore: util functions test routine
...
#get pre_rf results after running
...
def factor_n(x,n=3,flexible_factor=1):
    ...
    for _ in range(flexible_factor):
        #return factors of x, with length n
...
def arch_give_birth(input_str1,input_str2,input_arch_config_dict):
    #combine two input tiling factor combo  features in  all-layer scope
    #input_isolation
