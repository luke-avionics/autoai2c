...
def lo_random_pop(layer_list=[10,7]):
    ...
    for idx,size in enumerate(layer_list):
        for i in range(size-1,0,-1):
            actions.append(random.randint(0,i))
        actions.append(0)
    return actions
...
def lo_mutate(input_str1,prop,layer_list=[10,7]):
    ...
    if random.random()<=prop:
        ...
        random.shuffle(pos,random=random.random())
        pos=pos[0:size]
        ref_pos=[]
        ...
        for i in pos:
            if ref_pos[i]==0:
                ...
                str1=str1[0:i]+[random.randint(0,ref_pos[i])]+str1[i+1:]
    return str1
...
def arch_mutate(str1,prop, arch_factor_list,mutate_pos_num=4):
    ...
    for _ in range(len(arch_factor_list)):
        ...
        random.shuffle(tmp_pos,random=random.random())
    ...
    for indx in pos: 
        ...
        sub_pos=random.randint(0,len(arch_factor_list[layer][key])-1)
...
def performance_feedback(tiling1,pe_array,pe_array_dim_choices,param,tmp_hw_spec):
